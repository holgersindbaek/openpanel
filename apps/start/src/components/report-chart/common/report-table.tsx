    ...
    if (grouped && result.length > 0) {
      ...
      const sortFn = (
        ...
        for (const sort of sorting) {
          ...
          } else if (id === 'metric-percentage') {
            aValue = data.metrics.sum === 0 ? 0 : (a.sum / data.metrics.sum) * 100;
            bValue = data.metrics.sum === 0 ? 0 : (b.sum / data.metrics.sum) * 100;
    ...
    metrics.forEach((metric) => {
      ...
      if (metric.key === 'average') {
        cols.push({
          ...
          header: 'Percentage',
          accessorFn: (row) =>
            data.metrics.sum === 0 ? 0 : (row.sum / data.metrics.sum) * 100,
          enableSorting: true,
          size: 110,
          cell: ({ row }) => {
            const percentage =
              data.metrics.sum === 0
                ? 0
                : (row.original.sum / data.metrics.sum) * 100;
