    ...
    if (
      ...
      switch (operator) {
        ...
        case 'between': {
          ...
          const min = sqlstring.escape(String(value[0]).trim());
          const max = sqlstring.escape(String(value[1]).trim());
          if (isWildcard) {
            where[id] =
              `arrayExists(x -> toFloat64OrZero(x) BETWEEN toFloat64(${min}) AND toFloat64(${max}), ${whereFrom})`;
          } else {
            where[id] =
              `toFloat64OrZero(${whereFrom}) BETWEEN toFloat64(${min}) AND toFloat64(${max})`;
          }
          break;
        }
      }
      ...
      switch (operator) {
        ...
        case 'between': {
          ...
          if (isNumericColumn(name)) {
            where[id] =
              `toFloat64(${name}) BETWEEN toFloat64(${min}) AND toFloat64(${max})`;
          } else {
            where[id] = `${name} BETWEEN ${min} AND ${max}`;
          }
          break;
        }
      }
